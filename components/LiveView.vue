  ...
  <v-dialog v-model="dialog" persistent style="height: 100vh">
    <div>
      <v-btn @click="exitCam" fab light x-small id="exit-button" elevation="6">
        ...
        <svg xmlns="http://www.w3.org/2000/svg" style="width: 20px; height: 20px;" viewBox="0 0 320 512">
    ...
    <div v-if="status == 'online'">
      <div :id="cameraDivId" style="position: relative"></div>
    </div>
...
<script>
export default {
  props: ["cameraDivId", "cameraSrc", "dialog", "status", "thumbnail"],
  methods: {
    exitCam() {
      this.$emit("exit-cam");
    },
  ...
  mounted() {
    console.log(this.status, this.thumbnail);
  },
};
</script>
